...
export class User extends Entity<IUser>  {
    ...
    acceptFriendship(inviter: User): void {
        const invitationIndex = this.invitations.findIndex((invite) => invite.user.id === inviter.id);

        if (invitationIndex !== -1) {
            this._invitations.splice(invitationIndex, 1);
            this._friends.push({ user: inviter, status: FriendshipStatus.Accepted });
            const inviterIndex = inviter.friends.findIndex((friend) => friend.user.id === this.id);
            if (inviterIndex !== -1) {
    ...
    declineFriendship(inviter: User): void {
        this._invitations = this.invitations.filter((invite) => invite.user.id !== inviter.id);
